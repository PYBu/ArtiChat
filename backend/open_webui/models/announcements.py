...
DEFAULT_ANNOUNCEMENT_IMAGE_URL = 'https://github.com/PYBu/ArtiChat/blob/main/artivis-ass/title.png?raw=true'
...
class AnnouncementsTable:
    ...
    async def delete(self, announcement_id: str, db: AsyncSession | None = None) -> dict:
        async with get_announcement_db_context(db) as session:
            row = await session.get(Announcement, announcement_id)
            if row is None:
                raise ValueError('ANNOUNCEMENT_NOT_FOUND')
            await session.execute(delete(AnnouncementView).where(AnnouncementView.announcement_id == announcement_id))
