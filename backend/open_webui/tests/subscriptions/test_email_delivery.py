...
def test_smtp_settings_require_reentry_after_secret_key_rotation():
    encrypted = encrypt_smtp_password('mail-password', secret_key='old-secret')
    settings = {
        'enabled': True,
        'host': 'smtp.example.com',
        'port': 587,
    ...
    response = smtp_admin_settings(settings, secret_key='new-secret')
    assert response['password'] == ''
    assert response['password_configured'] is True
    assert response['password_requires_reset'] is True

    with pytest.raises(ValueError, match='SMTP_PASSWORD_REENTER_REQUIRED'):
        normalize_smtp_settings(
            {**settings, 'password': ''},
            current_encrypted_password=encrypted,
