...
async def test_legacy_announcement_fields_fill_presentation_defaults(db_session):
    ...
    assert announcement.image_url == 'https://github.com/PYBu/ArtiChat/blob/main/artivis-ass/title.png?raw=true'
...
@pytest.mark.asyncio
async def test_deleted_announcement_is_removed_from_admin_list_and_views(db_session):
    Announcements = announcements()
    announcement = await Announcements.create(
        title='Delete me',
    ...
    await Announcements.mark_viewed(announcement.id, user_id='user-1', db=db_session)
    deleted = await Announcements.delete(announcement.id, db=db_session)
    ...
    assert deleted == {'id': announcement.id, 'deleted': True}
    assert announcement.id not in [item.id for item in visible]
    assert announcement.id not in [item.id for item in history]
