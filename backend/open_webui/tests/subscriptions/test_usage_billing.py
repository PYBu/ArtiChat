...
@pytest.mark.asyncio
async def test_request_is_partially_billed_without_making_balance_negative(db_session):
    await SubscriptionPlans.seed_defaults(db=db_session)
    ...
    subscription = await UserSubscriptions.get_by_user_id('user-2', db=db_session)
    assert usage.status == 'partially_billed'
    assert usage.cost_micros == chatpoint_to_micros(2)
    assert usage.plan_cost_micros == chatpoint_to_micros(1)
    assert usage.unpaid_cost_micros == chatpoint_to_micros(1)
...
@pytest.mark.asyncio
async def test_total_only_usage_uses_the_highest_token_price(db_session):
    await SubscriptionPlans.seed_defaults(db=db_session)
    await ensure_subscription_current('total-only-user', now=1_720_000_000, db=db_session)

    usage = await bill_model_usage(
        user_id='total-only-user',
        model_id='total-only-model',
        quota_mode='metered',
        usage_multiplier='1',
        pricing={
            'input_chatpoint_per_million': '2',
            'output_chatpoint_per_million': '7',
            'cache_creation_chatpoint_per_million': '11',
            'cache_read_chatpoint_per_million': '3',
        },
        usage={'total_tokens': 1_000_000},
        metadata={},
        is_admin=False,
        request_id='total-only-request',
        now=1_720_000_001,
        db=db_session,
    )
    ...
    assert usage.cost_micros == chatpoint_to_micros(11)


@pytest.mark.asyncio
async def test_unclassified_total_difference_uses_the_highest_token_price(db_session):
    await SubscriptionPlans.seed_defaults(db=db_session)
    await ensure_subscription_current('unclassified-user', now=1_720_000_000, db=db_session)

    usage = await bill_model_usage(
        user_id='unclassified-user',
        model_id='unclassified-model',
        quota_mode='metered',
        usage_multiplier='1',
        pricing={
            'input_chatpoint_per_million': '2',
        ...
        metadata={},
        is_admin=False,
        request_id='unclassified-request',
        now=1_720_000_001,
        db=db_session,
    )
    ...
    assert usage.cost_micros == chatpoint_to_micros(13)


@pytest.mark.asyncio
async def test_merged_cache_usage_is_preserved_in_billing_snapshot(db_session):
    await SubscriptionPlans.seed_defaults(db=db_session)
    await ensure_subscription_current('merged-cache-user', now=1_720_000_000, db=db_session)
    ...
    usage = await bill_model_usage(
        user_id='merged-cache-user',
        model_id='merged-cache-model',
        quota_mode='metered',
        usage_multiplier='1',
        pricing={
            'input_chatpoint_per_million': '10',
        ...
        metadata={},
        is_admin=False,
        request_id='merged-cache-request',
        now=1_720_000_001,
        db=db_session,
    )
    ...
    assert usage.cache_creation_tokens == 500_000
    assert usage.cache_read_tokens == 150_000
    assert usage.total_tokens == 1_800_000
    assert usage.cost_micros == chatpoint_to_micros('15.15')
    assert usage.raw_usage['cache_creation_tokens'] == 500_000
    assert usage.raw_usage['cache_read_tokens'] == 150_000
...
@pytest.mark.asyncio
async def test_replayed_request_id_is_billed_once(db_session):
    await SubscriptionPlans.seed_defaults(db=db_session)
    await ensure_subscription_current('replayed-billing-user', now=1_720_000_000, db=db_session)

    async def bill_once():
        return await bill_model_usage(
            user_id='replayed-billing-user',
            model_id='metered-model',
            quota_mode='metered',
            usage_multiplier='1',
            pricing={
    ...
    first = await bill_once()
    second = await bill_once()
    subscription = await UserSubscriptions.get_by_user_id('replayed-billing-user', db=db_session)
    usage_summary = await SubscriptionUsages.get_usage_summary(
        user_id='replayed-billing-user', db=db_session
    )

    assert first.id == second.id
    assert len(usage_summary['items']) == 1
    assert subscription.plan_balance_micros == chatpoint_to_micros(90)


@pytest.mark.asyncio
async def test_concurrent_replayed_request_id_is_billed_once(db_session):
    await SubscriptionPlans.seed_defaults(db=db_session)
    await ensure_subscription_current('concurrent-replay-user', now=1_720_000_000, db=db_session)
    Session = async_sessionmaker(db_session.bind, expire_on_commit=False)

    async def bill_once():
        async with Session() as session:
            return await bill_model_usage(
                user_id='concurrent-replay-user',
                model_id='metered-model',
                quota_mode='metered',
                usage_multiplier='1',
                pricing={
                ...
                metadata={},
                is_admin=False,
                request_id='concurrent-replayed-request',
                now=1_720_000_001,
                db=session,
            )

    first, second = await asyncio.gather(bill_once(), bill_once())
    db_session.expire_all()
    subscription = await UserSubscriptions.get_by_user_id('concurrent-replay-user', db=db_session)
    usage_summary = await SubscriptionUsages.get_usage_summary(
        user_id='concurrent-replay-user', db=db_session
    )
