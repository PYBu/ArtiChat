...
@pytest.mark.asyncio
async def test_period_reset_rolls_back_when_ledger_insert_fails(db_session, monkeypatch):
    await SubscriptionPlans.seed_defaults(db=db_session)
    subscription = await ensure_subscription_current('reset-rollback-user', now=1_720_000_000, db=db_session)
    await UserSubscriptions.adjust_balances(
        'reset-rollback-user',
        plan_delta_micros=-chatpoint_to_micros(7),
        event_type='admin_adjustment',
    ...
    before = await UserSubscriptions.get_by_user_id('reset-rollback-user', db=db_session)

    async def fail_insert(*_args, **_kwargs):
        raise RuntimeError('ledger insert failed')

    monkeypatch.setattr(SubscriptionLedgers, 'insert', fail_insert)
    ...
    after = await UserSubscriptions.get_by_user_id('reset-rollback-user', db=db_session)
    assert after.period_start_at == before.period_start_at
    assert after.period_end_at == before.period_end_at
    assert after.plan_balance_micros == before.plan_balance_micros


@pytest.mark.asyncio
async def test_auto_downgrade_rolls_back_when_ledger_insert_fails(db_session, monkeypatch):
    await SubscriptionPlans.seed_defaults(db=db_session)
    start = 1_720_000_000
    before = await UserSubscriptions.create_from_plan(
        user_id='downgrade-rollback-user',
        plan_id=PLUS_TIER,
        starts_at=start,
        expires_at=start + 30 * 24 * 60 * 60,
        source='redemption',
    ...
    with pytest.raises(RuntimeError, match='ledger insert failed'):
        await ensure_subscription_current(
            'downgrade-rollback-user',
            now=before.expires_at + 1,
            db=db_session,
        )

    db_session.expire_all()
    after = await UserSubscriptions.get_by_user_id('downgrade-rollback-user', db=db_session)
    assert after.tier == PLUS_TIER
