...
def test_edit_tool_falls_back_to_current_image_attachment(monkeypatch):
    ...
    assert edit_call.await_args.kwargs['form_data'].image == ['source-1']
    ...
    chat = SimpleNamespace(
        chat={
            'history': {
                'currentId': 'user-2',
                'messages': {
                    'user-1': {'id': 'user-1', 'parentId': None, 'role': 'user', 'content': 'Generate'},
                    'assistant-1': {
                        'id': 'assistant-1',
                        'parentId': 'user-1',
                        'role': 'assistant',
...
def test_openai_edit_receives_resolved_image_bytes_as_multipart(monkeypatch):
    ...
    monkeypatch.setattr(images, 'get_image_config', AsyncMock(return_value=_image_config()))
...
def test_internal_image_operation_honors_global_switch(monkeypatch):
    ...
    with pytest.raises(HTTPException) as generation_error:
        asyncio.run(
            images.assert_image_operation_access(
