...
const settingsModalPath = 'src/lib/components/chat/SettingsModal.svelte';
const adminSettingsModalPath = 'src/lib/components/admin/AdminSettingsModal.svelte';
...
for (const forbidden of ['AdminUpdate', "id: 'admin:update'", "selectedTab === 'admin:update'"]) {
	if (settingsModal.includes(forbidden)) {
		throw new Error(`Personal settings modal contains admin update control ${forbidden}`);
	}
}

const adminSettingsModal = fs.readFileSync(adminSettingsModalPath, 'utf8');
for (const required of [
	"import Settings from './Settings.svelte'",
	"$user?.role !== 'admin'",
	'<Settings'
]) {
	if (!adminSettingsModal.includes(required)) {
		throw new Error(`Admin settings modal is missing ${required}`);
	}
}
