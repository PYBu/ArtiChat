...
const files = [
	// README.md is user-maintained and intentionally includes upstream provenance and licensing notes.
	'TROUBLESHOOTING.md',
	'docs/SECURITY.md',
	...
	'backend/open_webui/utils/notifications.py',
...
const compatibilityAllowlist = new Map([
	['src/lib/components/AddTerminalServerModal.svelte', ['ghcr.io/open-webui/open-terminal:latest']],
	['src/lib/components/admin/Settings/Documents.svelte', ['X-OpenWebUI-File-Id']],
	['src/lib/components/layout/Sidebar/RecursiveFolder.svelte', ['application/x-open-webui-drag']],
	['src/lib/components/layout/Sidebar/PinnedNoteList.svelte', ['application/x-open-webui-drag']],
	['src/lib/components/layout/Sidebar/ChatItem.svelte', ['application/x-open-webui-drag']],
	['src/lib/components/chat/MessageInput.svelte', ['application/x-open-webui-drag']]
]);
...
for (const file of files) {
	...
	for (const [index, line] of lines.entries()) {
		for (const label of forbidden) {
			const allowedMarkers = compatibilityAllowlist.get(file) ?? [];
			const isCompatibilityIdentifier = allowedMarkers.some((marker) => line.includes(marker));
			if (line.includes(label) && !isCompatibilityIdentifier) {
...
if (fs.existsSync(path.join(root, 'src/lib/components/common/ThemeLogo.svelte'))) {
	...
	for (const marker of ['branding?.logo_light', 'branding?.logo_dark']) {
		if (!themeLogo.includes(marker))
			findings.push(`ThemeLogo missing platform branding marker ${marker}`);
	}
	...
	for (const marker of [
		'$config?.branding?.about_title',
		'$config?.branding?.about_content',
		'DEFAULT_ABOUT_TITLE',
