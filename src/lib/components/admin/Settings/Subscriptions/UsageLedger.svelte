...
<div class="flex flex-col gap-5">
	<div class="flex flex-wrap items-start justify-between gap-3">
		...
		<button type="button" class="rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-gray-100 dark:hover:bg-gray-850" on:click={load}>刷新</button>
	</div>

	<div class="grid gap-2 md:grid-cols-3 xl:grid-cols-6">
		<input class="rounded-lg border border-gray-100 bg-transparent px-3 py-2 text-xs dark:border-gray-850" bind:value={userFilter} placeholder="用户 ID" />
		<input class="rounded-lg border border-gray-100 bg-transparent px-3 py-2 text-xs dark:border-gray-850" bind:value={modelFilter} placeholder="模型 ID" />
		<select class="rounded-lg border border-gray-100 bg-transparent px-3 py-2 text-xs dark:border-gray-850" bind:value={statusFilter}>
		...
		<input type="date" class="rounded-lg border border-gray-100 bg-transparent px-3 py-2 text-xs dark:border-gray-850" bind:value={startDate} />
		<input type="date" class="rounded-lg border border-gray-100 bg-transparent px-3 py-2 text-xs dark:border-gray-850" bind:value={endDate} />
		<div class="flex gap-2">
			<button type="button" class="flex-1 rounded-lg bg-black px-3 py-2 text-xs text-white dark:bg-white dark:text-black" on:click={load}>筛选</button>
			<button type="button" class="rounded-lg px-3 py-2 text-xs hover:bg-gray-100 dark:hover:bg-gray-850" on:click={resetFilters}>重置</button>
		</div>
	</div>
	...
	{:else}
		<div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
			{#each [
				['总扣费 CP', formatChatpoint(usage.total_cost_micros)],
				['输入 Token', formatNumber(usage.total_input_tokens ?? 0)],
				['输出 Token', formatNumber(usage.total_output_tokens ?? 0)],
				['创建缓存', formatNumber(usage.total_cache_creation_tokens ?? 0)],
				['读取缓存', formatNumber(usage.total_cache_read_tokens ?? 0)]
			] as summary}
				...
				<div class="overflow-x-auto border-y border-gray-100 dark:border-gray-850">
					<table class="min-w-[1280px] w-full text-left text-xs">
						<thead class="text-gray-500">
							<tr>{#each ['用户', '模型', '输入', '输出', '创建缓存', '读取缓存', '消耗 CP', '首字延迟', '总耗时', '时间', 'IP', '状态'] as heading}<th class="px-2 py-2 font-medium">{heading}</th>{/each}</tr>
						</thead>
				...
				<div class="divide-y divide-gray-100 border-y border-gray-100 dark:divide-gray-850 dark:border-gray-850">
					{#each ledger as entry}
						<div class="grid gap-2 py-2 text-xs md:grid-cols-[1fr_8rem_7rem_7rem_10rem]">
							<div class="truncate">{userLabel(entry)}</div><div>{entry.event_type}</div><div>{formatChatpoint(entry.plan_delta_micros)} 周期</div><div>{formatChatpoint(entry.check_delta_micros)} 充值</div><div class="md:text-right">{formatDate(entry.created_at)}</div>
						</div>
					{/each}
				</div>
			{:else}
				<div class="border-y border-gray-100 py-4 text-gray-500 dark:border-gray-850">暂无余额记录。</div>
			{/if}
		</div>
	{/if}
</div>
