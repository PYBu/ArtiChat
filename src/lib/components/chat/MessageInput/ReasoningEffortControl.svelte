<script lang="ts">
	...
	let usesModelDefault = level === DEFAULT_REASONING_LEVEL;
	...
	const setLevelIndex = (nextIndex: number, modelDefault = false) => {
		...
		>
			<div class="panel-head">
				<div class="title-line">
					...
					{#if usesModelDefault}
						<span class="default-chip">{$i18n.t('Default')}</span>
					{/if}
				</div>
				...
				<div class="head-actions">
					...
					<Tooltip content={$i18n.t('Reset to default')} placement="top">
						<button
							...
							on:click={() => setLevelIndex(getReasoningLevelIndex(DEFAULT_REASONING_LEVEL), true)}
						>
