...
if [[ "$*" == *" exec -T artichat python -"* ]]; then
  if [[ "${FAKE_EXTERNAL_PERSISTENCE:-false}" == "true" ]]; then
    echo 'automatic update refused: external persistence is not covered' >&2
    exit 1
  fi
  exit 0
fi
...
EOF
  ...
  export FAKE_EXTERNAL_PERSISTENCE=false
}
...
if "$DEPLOY_SCRIPT" 0.1.2 operation-external-persistence; then
  fail 'external persistence deployment unexpectedly succeeded'
fi
grep -q '"stage": "failed"' "$ARTICHAT_UPDATE_STATE_FILE" || fail 'external persistence refusal was not recorded'
if grep -Eq '(^| )pull | stop artichat$' "$FAKE_DOCKER_LOG"; then
  fail 'external persistence refusal happened after deployment mutation'
fi
test "$(find "$ARTICHAT_BACKUP_DIR" -type f | wc -l)" -eq 0 || fail 'external persistence refusal created a misleading backup'
