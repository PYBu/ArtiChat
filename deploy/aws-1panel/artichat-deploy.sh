...
verify_ready() {
  local payload
  payload="$(curl -fsS "$HEALTH_URL/ready")" || return 1
  python3 - "$payload" <<'PY'
import json
import sys
...
verify_release() {
  ...
  while (( SECONDS <= deadline )); do
    if verify_ready && actual="$(read_version)" && [[ "$actual" == "$expected" ]]; then
      return 0
    fi
...
preflight_local_persistence() {
  compose exec -T artichat python - <<'PY'
import os
import sys
from pathlib import Path
...
data_dir = Path(os.environ.get('DATA_DIR', '/app/backend/data')).resolve()
unsupported = []
...
if database_url:
    ...
    if not scheme.startswith('sqlite'):
        unsupported.append('primary database')
    else:
        database_path = unquote(urlsplit(database_url).path)
        if database_path in {'', '/:memory:'}:
            unsupported.append('non-durable SQLite database')
        else:
            resolved_database = Path(database_path).resolve()
            try:
                resolved_database.relative_to(data_dir)
            except ValueError:
                unsupported.append('SQLite database outside /app/backend/data')

vector_db = os.environ.get('VECTOR_DB', 'chroma').strip().lower() or 'chroma'
if vector_db != 'chroma':
    unsupported.append(f'vector store ({vector_db})')

storage_provider = os.environ.get('STORAGE_PROVIDER', 'local').strip().lower() or 'local'
if storage_provider != 'local':
    unsupported.append(f'file storage ({storage_provider})')

if unsupported:
    print(
        'automatic update refused: the local ArtiChat backup does not cover '
        + ', '.join(unsupported),
        file=sys.stderr,
    )
    print(
        'Back up and rehearse restore for every external persistence service, then use a manual upgrade runbook.',
        file=sys.stderr,
    )
    raise SystemExit(1)
PY
...
[[ -s "$BACKUP_TEMPORARY" ]]
tar --force-local -tzf "$BACKUP_TEMPORARY" >/dev/null
