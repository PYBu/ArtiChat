...
内置自动更新与自动回滚只支持其备份能够完整覆盖的默认组合：位于 `/app/backend/data` 下的 SQLite（或 SQLCipher SQLite）、Chroma 向量库和本地文件存储。更新脚本会在拉取镜像或停止服务前检查当前容器；检测到外部 PostgreSQL、其他外部向量库、S3/GCS/Azure 文件存储，或数据目录之外的 SQLite 时会拒绝自动更新。

使用外部持久层的服务商必须先分别完成数据库、向量库和对象存储的一致性备份及恢复演练，再按人工升级手册更新。仅有 `/data/artichat-prod/data` 压缩包不能证明这些外部数据可回滚，也不要通过关闭门禁把不完整备份当作可恢复备份。
